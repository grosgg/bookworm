...
export default async function BookCard({ book }: { book: BookType }) {
  ...
  return (
    <div className="rounded-lg shadow-md bg-white p-6 border border-gray-200 hover:shadow-lg transition-shadow flex items-start flex-col">
      <div className="flex flex-col md:flex-row md:w-full">
        ...
        {book.coverUrl && (
          <img
            src={book.coverUrl}
            alt={book.title}
            className="w-24 h-36 object-cover rounded shadow my-2"
          />
        )}
      </div>
